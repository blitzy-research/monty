...
// === Timeout enforcement across a callable-driven iterator advance ===
// Advancing an `iter(callable, sentinel)` iterator re-enters the interpreter, so it is the only
// iterator advance that can push a frame and run a nested `run()` loop. A time limit is raised at an
// instruction boundary of that nested loop and returns by `?` without unwinding, which leaves the
// callee frames registered while `Opcode::ForIter` still has to pop the iterator it left on the
// operand stack. Popping by position with those frames live desynchronises the stack against
// `CallFrame::stack_base` and used to abort the process with a Rust panic instead of surfacing a
// Python-level `TimeoutError` - a sandbox-containment failure, since untrusted code must never crash
// the host. `Opcode::ForIter` restores the frame depth before popping, and these tests pin that.
//
// Each case runs several trials because which loop - outer or nested - observes the expired deadline
// depends on where the rate-limited time check lands.

/// Helper: drives `code` under a short wall-clock limit repeatedly and requires every run to end in a
/// Python-level `TimeoutError`. A regression re-introduces a Rust panic, which fails the test.
fn assert_repeated_timeout(code: &str, label: &str) {
    for trial in 0..25 {
        let ex = MontyRun::new(code.to_owned(), "test.py", vec![]).unwrap();
        let limits = ResourceLimits::new().max_duration(Duration::from_millis(10));
        let result = ex.run(vec![], LimitedTracker::new(limits), PrintWriter::Stdout);

        let exc = result.expect_err(&format!("{label}: trial {trial} should exceed the time limit"));
        assert_eq!(
            exc.exc_type(),
            ExcType::TimeoutError,
            "{label}: trial {trial} expected TimeoutError, got: {exc}"
        );
    }
}

/// A runaway callable driven by a `for` loop must surface a `TimeoutError`, never a panic.
///
/// The callable never returns, so the deadline always expires inside the nested `run()` loop.
#[test]
fn timeout_in_for_over_callable_sentinel_iterator() {
    let code = r"
def never():
    while True:
        pass
...
for v in iter(never, 0):
    pass
";
    assert_repeated_timeout(code, "for over iter(callable, sentinel)");
}

/// The same guarantee when the callable returns normally, so the loop re-enters the VM every step.
#[test]
fn timeout_in_for_over_returning_callable_sentinel_iterator() {
    let code = r"
c = [0]
...
for v in iter(never, 0):
    pass
";
    assert_repeated_timeout(code, "for over a returning iter(callable, sentinel)");
}

/// The same guarantee when the `for` loop runs inside a function frame rather than at module level.
#[test]
fn timeout_in_nested_frame_for_over_callable_sentinel_iterator() {
    let code = r"
c = [0]
...
drive()
";
    assert_repeated_timeout(code, "for over iter(callable, sentinel) inside a function");
}

/// The `next()` drive path shares the advance but not the operand-stack cleanup, so it must stay clean.
#[test]
fn timeout_in_next_over_callable_sentinel_iterator() {
    let code = r"
c = [0]
...
while True:
    next(it)
";
    assert_repeated_timeout(code, "while/next over iter(callable, sentinel)");
}

/// An allocation limit tripped by the callable must still surface a `MemoryError`.
///
/// Allocation failures inside the nested loop are routed through `handle_exception`, which unwinds the
/// frames itself, so this covers the neighbouring path where the frame depth is already restored when
/// the error reaches `Opcode::ForIter`. The limit is on the allocation *count*, which only ever grows,
/// because a per-step value that the loop immediately releases keeps live memory flat.
#[test]
fn allocation_limit_in_for_over_callable_sentinel_iterator() {
    let code = r"
c = [0]
...
for v in iter(grow, 0):
    pass
";
    let ex = MontyRun::new(code.to_owned(), "test.py", vec![]).unwrap();

    let limits = ResourceLimits::new().max_allocations(2000);
    let result = ex.run(vec![], LimitedTracker::new(limits), PrintWriter::Stdout);

    let exc = result.expect_err("should exceed the allocation limit");
    assert_eq!(exc.exc_type(), ExcType::MemoryError, "expected MemoryError, got: {exc}");
}

/// A recursion limit tripped inside the callable must still surface a `RecursionError`.
///
/// `RecursionError` is the one catchable resource error, so this is the catchable counterpart of the
/// timeout cases: it unwinds inside the nested loop and reaches `Opcode::ForIter` with the frame depth
/// already restored.
#[test]
fn recursion_limit_in_for_over_callable_sentinel_iterator() {
    let code = r"
def deep(n):
    return deep(n + 1)
...
";
    ...
    let exc = result.expect_err("should exceed the recursion limit");
    assert_eq!(
        exc.exc_type(),
        ExcType::RecursionError,
        "expected RecursionError, got: {exc}"
    );
}
