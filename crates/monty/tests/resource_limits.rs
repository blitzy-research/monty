...
/// Program whose final two heap allocations are the pair tuple and the iterator entry created by
/// `iter(callable, sentinel)`.
///
/// Both arguments are heap values on purpose — a function carrying a default is a
/// `HeapData::FunctionDefaults` entry and the sentinel is a list — so the pair tuple really does
/// hold two references and a rejected allocation has something it could strand. The allocation
/// ladder is therefore: the empty tuple singleton, the function's defaults, the sentinel list, the
/// pair tuple, and finally the iterator entry. Lowering `max_allocations` one step at a time is what
/// lets the two tests below target one specific allocation each, and the resource error carries the
/// rejected allocation's ordinal so each test can prove *which* allocation it rejected.
const ITER_SENTINEL_CONSTRUCTION: &str = r"
def read(tag='s'):
    return 0

sentinel = ['STOP']
it = iter(read, sentinel)
1
";

/// Runs [`ITER_SENTINEL_CONSTRUCTION`] with `max_allocations` set to `limit`.
fn run_iter_sentinel_construction(limit: usize) -> Result<MontyObject, monty::MontyException> {
    let ex = MontyRun::new(ITER_SENTINEL_CONSTRUCTION.to_owned(), "test.py", vec![]).unwrap();
    let limits = ResourceLimits::new().max_allocations(limit);
    ex.run(vec![], LimitedTracker::new(limits), PrintWriter::Stdout)
}

/// Returns the smallest `max_allocations` limit at which [`ITER_SENTINEL_CONSTRUCTION`] completes,
/// which is also the number of allocations the program makes.
///
/// # Foot-gun: the search must walk downwards
///
/// It stops at the first rejection, so it never runs the program at a limit below the one that
/// rejects the iterator allocation. That is deliberate: rejecting the *pair tuple* strands two
/// references, which aborts the whole test binary under `ref-count-panic` (see
/// [`iter_callable_sentinel_pair_allocation_rejected`] for why that is inherited behaviour), so an
/// upward search would take every other test in this file down with it.
fn smallest_iter_sentinel_construction_limit(upper: usize) -> usize {
    assert!(
        run_iter_sentinel_construction(upper).is_ok(),
        "the upper bound must be a limit the program fits within"
    );
    let mut smallest = upper;
    for limit in (1..upper).rev() {
        if run_iter_sentinel_construction(limit).is_err() {
            break;
        }
        smallest = limit;
    }
...
/// Rejecting the *iterator entry* — the last allocation `iter(callable, sentinel)` makes — must
/// release the pair tuple, and through it both the callable and the sentinel.
///
/// `MontyIter::allocate_callable_sentinel` allocates that entry reference-free and moves the pair
/// reference in afterwards through the infallible `HeapRead::get_mut`, precisely so a rejection here
/// leaves the pair reference owned by the constructor and releasable with `drop_with_heap`. This
/// test runs under every feature configuration, including `ref-count-panic`, where `Value`'s `Drop`
/// panics for any `Ref` dropped without heap access — passing there is the evidence that the
/// release really happens rather than the references simply being forgotten.
#[test]
fn iter_callable_sentinel_iterator_allocation_rejected_releases_pair() {
    let smallest = smallest_iter_sentinel_construction_limit(16);
    assert!(
        smallest >= 3,
        "construction must allocate at least the singleton, the pair tuple and the iterator entry, \
         got a total of {smallest}"
    );

    let exc = run_iter_sentinel_construction(smallest - 1).expect_err("the iterator entry must be rejected");
    assert_eq!(exc.exc_type(), ExcType::MemoryError);
    let expected = format!("allocation limit exceeded: {smallest} > {}", smallest - 1);
    assert!(
        exc.message().is_some_and(|m| m.contains(&expected)),
        "expected the last allocation (the iterator entry) to be rejected with {expected:?}, got: {exc}"
    );
}

/// Rejecting the *pair tuple* allocation reports the resource error cleanly, but cannot release the
/// two references the rejected tuple was carrying.
///
/// `Heap::allocate` takes its `HeapData` by value and only then runs the fallible resource check, so
/// a rejected `HeapData::Tuple` is destroyed by ordinary Rust destruction, which has no heap access
/// and therefore cannot decrement the callable's or the sentinel's refcount. There is no in-crate
/// way for the constructor to avoid that: `HeapData::Tuple` is immutable by construction —
/// `Tuple::new` and its `items` field are private to `types/tuple.rs` and its `contains_refs` flag
/// is fixed at creation — so a reference-free tuple cannot be allocated first and filled afterwards
/// the way the iterator entry is, and a pre-flight budget check is not available either because
/// `ResourceTracker::on_allocate` updates the very counters it checks. This is the same inherited
/// behaviour as every other owned-value allocation in the crate, it is documented on
/// `MontyIter::init`, and it is why this test shares the ignore annotation used by the resource
/// exhaustion tests above: under `ref-count-panic` the stranded references trip `Value`'s `Drop`
/// guard. The assertion still pins the useful half of the contract — the interpreter reports
/// `MemoryError` for the pair allocation and stops there rather than building a half-formed
/// iterator.
#[test]
#[cfg_attr(
    feature = "ref-count-panic",
...
fn iter_callable_sentinel_pair_allocation_rejected() {
    let smallest = smallest_iter_sentinel_construction_limit(16);
    assert!(
        smallest >= 4,
        "construction must allocate the callable, the sentinel, the pair tuple and the iterator \
         entry, got a total of {smallest}"
    );

    let exc = run_iter_sentinel_construction(smallest - 2).expect_err("the pair tuple must be rejected");
    assert_eq!(exc.exc_type(), ExcType::MemoryError);
    let expected = format!("allocation limit exceeded: {} > {}", smallest - 1, smallest - 2);
    assert!(
        exc.message().is_some_and(|m| m.contains(&expected)),
        "expected the second-to-last allocation (the pair tuple) to be rejected with {expected:?}, got: {exc}"
    );
}

/// A collection reached *inside* a step must not sweep the temporary iterator, its pair tuple, or
/// the default value `next()` is holding for the exhausted case.
///
/// Advancing a callable-driven iterator re-enters the interpreter, which makes it the only iterator
/// whose advance can trigger a collection. The collector marks from the operand stack, globals and
/// the exception stack, and `next()` pops its arguments before running, so an iterator that is never
/// bound to a name — together with the callable and sentinel it owns through its pair tuple, and the
/// default handed to `next()` — is reachable only from the interpreter's own Rust locals while the
/// callable runs. `MontyIter::callable_sentinel_step` therefore performs the nested call inside
/// `VM::with_gc_paused`.
///
/// `iter__callable_sentinel_gc.py` covers the observable behaviour under both interpreters. This
/// test adds the piece a fixture cannot express: `allocations_since_gc` proves a collection actually
/// ran, so the churn below can never silently stop exercising the suspension, and the heap count
/// proves the churn's garbage was reclaimed while the iterator's own graph was not leaked.
#[test]
#[cfg(feature = "ref-count-return")]
fn iter_callable_sentinel_survives_collection_during_step() {
    // Two steps, each allocating 132,000 reference-holding containers, so the collector's 100,000
    // allocation interval is crossed inside a step both times. The first step's iterator is driven
    // with a freshly built default that nothing else refers to; the second is driven with a bound
    // default whose identity is then asserted, so a swept-and-reallocated default cannot pass.
    let code = r"
state = {'calls': 0}
sentinel = ['STOP']
holder = ['DEFAULT']
...
unrooted = next(iter(step, sentinel), ['FALLBACK'])
assert unrooted == ['FALLBACK'], 'an unrooted default survives a collection during the terminal step'
unrooted = None
rooted = next(iter(step, sentinel), holder)
assert rooted is holder, 'the exact default object is returned after a collection'
rooted = None
state['calls']
";
    let ex = MontyRun::new(code.to_owned(), "test.py", vec![]).unwrap();

    let output = ex.run_ref_counts(vec![]).expect("the program should run to completion");

    assert_eq!(output.py_object, MontyObject::Int(2), "both steps must have run");
    assert!(
        output.allocations_since_gc < 100_000,
        "a collection must have run inside a step: allocations_since_gc = {}",
        output.allocations_since_gc
    );
    assert!(
        output.heap_count < 20,
        "the churn's garbage must be reclaimed and nothing else leaked: {} heap objects",
        output.heap_count
    );
    assert_eq!(
        output.counts.get("sentinel"),
        Some(&1),
        "releasing both temporary iterators must release the pair tuple's reference to the sentinel"
    );
    assert_eq!(
        output.counts.get("holder"),
        Some(&1),
        "the default value must be left with only its own binding"
    );
}

/// An error raised by the sentinel comparison must release the value the callable just produced.
///
/// `MontyIter::callable_sentinel_step` holds the produced value in a `HeapGuard` across
/// `Value::py_eq`, whose only failure mode is `ResourceError::Recursion`. The program below forces
/// that failure on every step by comparing two deeply nested lists after priming the recursion
/// budget with cheap interpreter frames, so the comparison — not the priming and not the callable —
/// is what exceeds the limit. Running it with two very different repeat counts and requiring the
/// same live heap afterwards is what proves the guard releases the produced value: each step builds
/// a twenty-deep nest, so leaking it would make the live heap grow with the repeat count.
#[test]
#[cfg(feature = "ref-count-return")]
fn iter_callable_sentinel_comparison_error_releases_produced_value() {
    // `run_ref_counts` uses `NoLimitTracker`, whose recursion limit is 1000 frames. Priming with 990
    // interpreter frames leaves a budget only a few levels deep, which the twenty-deep comparison
    // then exceeds; priming with interpreter frames rather than nesting the data deeper keeps the
    // native stack shallow, which matters because a 1000-deep comparison overflows it in debug
    // builds. If either the limit or this priming depth ever drifts, the `assert False` below fires
    // rather than the test silently passing.
    let program = |repeats: usize| {
        format!(
            r"
sentinel = 'x'
for _ in range(20):
...
it = iter(nest, sentinel)


def one_step():
    try:
        next(it)
        assert False, 'the sentinel comparison must exceed the recursion limit'
...
"
    ...
    let mut heap_counts = Vec::new();
    for repeats in [10usize, 200] {
        let ex = MontyRun::new(program(repeats), "test.py", vec![]).unwrap();
        let output = ex.run_ref_counts(vec![]).expect("the program should run to completion");

        assert_eq!(
            output.py_object.to_string(),
            format!("[{repeats}, {repeats}]"),
            "every step must make exactly one call and fail in the comparison"
        );
        assert_eq!(
            output.counts.get("sentinel"),
            Some(&2),
            "the sentinel is owned twice: by its binding and by the iterator's pair tuple"
        );
        heap_counts.push(output.heap_count);
    }

    assert_eq!(
        heap_counts[0], heap_counts[1],
        "a comparison error must not accumulate live heap objects: {} objects after 10 errors versus \
         {} after 200",
        heap_counts[0], heap_counts[1]
    );
    assert!(
        heap_counts[0] < 40,
        "only the sentinel's nest, the iterator and its pair tuple should be live: {} heap objects",
        heap_counts[0]
    );
}

/// The callable and the sentinel are owned through the iterator's pair tuple, and releasing the
/// iterator releases all three.
///
/// This pins the ownership edge the collector depends on: `collect_child_ids` traces a
/// `HeapData::Iter` through `MontyIter::value` alone, so both owned values have to hang off that one
/// reference. Holding the iterator therefore gives the callable and the sentinel a count of two —
/// their own binding plus the pair tuple — with exactly one extra unnamed heap object, the pair
/// tuple itself. Dropping the last binding to the iterator takes the pair tuple and both of those
/// references with it.
#[test]
#[cfg(feature = "ref-count-return")]
fn iter_callable_sentinel_pair_owns_callable_and_sentinel() {
    let retained = r"
def read(tag='s'):
    return 0
...
sentinel = ['STOP']
it = iter(read, sentinel)
assert next(it) == 0, 'the iterator yields the produced value'
1
";
    let released = r"
def read(tag='s'):
...
";
    ...
    let held = MontyRun::new(retained.to_owned(), "test.py", vec![])
        .unwrap()
        .run_ref_counts(vec![])
        .expect("the program should run to completion");
    assert_eq!(
        held.counts.get("read"),
        Some(&2),
        "a retained iterator owns its callable through the pair tuple"
    );
    assert_eq!(
        held.counts.get("sentinel"),
        Some(&2),
        "a retained iterator owns its sentinel through the pair tuple"
    );
    assert_eq!(
        held.counts.get("it"),
        Some(&1),
        "the iterator itself is owned only by its binding"
    );
    assert_eq!(
        held.heap_count - held.unique_refs,
        1,
        "exactly one live heap object is unnamed: the pair tuple"
    );

    let dropped = MontyRun::new(released.to_owned(), "test.py", vec![])
        .unwrap()
        .run_ref_counts(vec![])
        .expect("the program should run to completion");
    assert_eq!(
        dropped.counts.get("read"),
        Some(&1),
        "releasing the iterator releases the pair tuple's reference to the callable"
    );
    assert_eq!(
        dropped.counts.get("sentinel"),
        Some(&1),
        "releasing the iterator releases the pair tuple's reference to the sentinel"
    );
    assert_eq!(
        dropped.heap_count, dropped.unique_refs,
        "the iterator and its pair tuple are both gone, so every live object is named"
    );
}
