...
# releasing it takes an explicit rebind of the name to an immediate. Sentinel equality
# alone exhausts it: every propagated exception leaves it live, and once the sentinel
# has been seen exhaustion is sticky and no further value is produced.
attempts = [0]
retained = ['retained']
...
# Transparency is unconditional, so a callable-raised StopIteration arrives with its
# own message rather than as an empty exhaustion signal and leaves the iterator usable
# like any other exception. CPython treats it as exhaustion instead, so this assertion
# lives in this Monty-only ref-counts fixture rather than in iter__callable_sentinel.py.
try:
    next(probe)
